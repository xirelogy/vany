...
import VanySwitchRenderRequest from './requests/VanySwitchRenderRequest';
...
const props = withDefaults(defineProps<{
   ...
   */
  codec?: VanyCodec<T, boolean>|null,
  /**
   * If disabled
   * @defaultValue false
   */
...
const emits = defineEmits<{
   ...
   * Change event
   */
  'change': [value: T|Error|null, rawValue?: boolean|null, context?: any],
}>();
...
// Create the host
const serviceHost = createVanyFormControlRenderServiceHost<T, boolean>({
  currentValue: props.modelValue,
  onUpdateValueFn: (value: T|null) => {
    emits('update:modelValue', value);
  },
  fwdFocus,
  codec: props.codec ?? _cast<VanyCodec<T, boolean>>(new VanyDummyCodec<boolean>),
  registeredControl: renderService,
  defaultValidateTrigger: 'change',
  controlNature: 'select',
});

serviceHost.onChangeEvent((value: T|Error|null, rawValue?: boolean|null, context?: any) => {
...
//#region Renderer
const render = () => {
  const request: VanySwitchRenderRequest = {
    vanyClass: 'switch',
